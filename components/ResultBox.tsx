...
const ResultBox = forwardRef<
  HTMLDivElement,
  { result: ToolComputationResult | null; locale?: Locale }
>(function ResultBox({ result, locale = DEFAULT_LOCALE }, ref) {
      ...
      <div aria-live="polite" aria-atomic="true">
        {!result ? (
          <div className="rounded-2xl border-2 border-dashed border-violet-200/55 bg-white/35 p-6 text-center text-sm text-slate-500 shadow-[0_4px_24px_rgba(99,102,241,0.06)] backdrop-blur-md">
            <p className="font-medium text-slate-600">{ws.resultEmptyTitle}</p>
            <p className="mt-1">{ws.resultEmptyBody}</p>
          </div>
        ) : (
          <div
