#!/usr/bin/env node
/**
 * High-confidence programmatic blog: GET automation-bundle → generate-blog → quality gate → PR.
 */
...
async function main() {
  ...
  const pick = bundle?.programmaticPick;
  if (!pick?.idea) {
    pipelineLog({ step: "abort", reason: "no_programmatic_pick" });
    process.exit(0);
  }

  const primaryKeyword = pick.idea.primaryKeyword;
  const topic = `Programmatic (${pick.confidence}): ${pick.idea.primaryKeyword}`.slice(0, 220);
  const variationSeed = `programmatic|${pick.path}|${pick.idea.kind}|${new Date().toISOString().slice(0, 10)}`;
  const behaviorPath = pick.path.startsWith("/") ? pick.path : undefined;
  const relatedToolSlugs = toolSlugsFromPath(pick.path);
  ...
  const branch = `content-engine/programmatic-${slug}-${Date.now().toString(36)}`;
  ...
  runGit([
    "commit",
    "-m",
    `content-engine: programmatic blog ${slug}\n\nParent: ${pick.path}. Idea: ${pick.idea.kind}. Quality: ${quality.score}.`,
  ]);
  runGit(["remote", "set-url", "origin", authRemote]);
  runGit(["push", "-u", "origin", branch]);

  const pr = await createPullRequest(owner, repo, token, {
    title: `Programmatic blog: ${title}`,
    head: branch,
    base: baseBranch,
    body: [
      "## Programmatic expansion (review required)",
      "",
      `- **Parent URL:** \`${pick.path}\``,
      `- **Idea kind:** ${pick.idea.kind}`,
      `- **Confidence:** ${pick.confidence}`,
      `- **Quality score:** ${quality.score}`,
      `- **Primary keyword:** ${primaryKeyword}`,
      "",
      "_Quality gate passed; human review still required before merge._",
    ].join("\n"),
  });
