...
export const metadata: Metadata = categoryLandingMetadata({
  ...
  title: "Free Real Estate Calculators: Mortgage, Rent & ROI",
  description:
    "Free real estate calculators for rent vs buy, mortgage affordability, rental yield, property ROI, and moving costs. Compare scenarios on HTTPS Toollabz pages with related-tool discovery.",
});
...
export default function RealEstateToolsPage() {
  ...
  const breadcrumbLd = breadcrumbJsonLd([
    ...
    { name: "Real estate tools", path: "/real-estate-tools" },
  ]);
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
      <nav className="mb-6 flex flex-wrap items-center gap-1 text-sm text-slate-500" aria-label="Breadcrumb">
        ...
        <ChevronRight className="h-3.5 w-3.5 shrink-0 opacity-40" aria-hidden />
        <span className="font-medium text-slate-700">Real Estate Tools</span>
      </nav>
      ...
      <header className={`mb-10 p-6 sm:p-8 ${toolGlassPanel}`}>
        ...
        <h1 className="mt-2 text-3xl font-extrabold tracking-tight text-slate-900 sm:text-4xl">Real estate tools</h1>
